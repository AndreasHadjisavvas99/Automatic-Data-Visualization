...
from features import uniqueness_features, s_pairwise_features, categorical_features, g_pairwise_features, quantitive_features,sequence_features
import sys 

#split x and y calculate features and split labels
def process_json_files(file_name):
    unique_entries = set()
    dtv_features = []
    ...
    for folder_path in folder_paths:
        ...
        for file_name in os.listdir(folder_path):
            ...
            #if file_name.endswith('.json') and file_name == 'its_communications_56.json':    # for debugging
                file_path = os.path.join(folder_path, file_name)
                with open(file_path, 'r', encoding='utf-8') as f:
                    content = json.load(f)
                    f_data_entry = content['data'][0]
                    plot_type = f_data_entry.get('type')
                    if folder_path == '../data1/line' and plot_type == 'scatter':
                            if 'mode' in f_data_entry and f_data_entry['mode'] == 'lines':
                                plot_type = 'line'
                    try:
                        if plot_type == 'pie':
                    ...
                    if plot_type != 'histogram':
                        if is_wrong(y_values):
                            continue
                    #type
                    data_type_x = categorize(x_values,file_path)
                    if plot_type != 'histogram':
                        data_type_y = categorize(y_values,file_path)
                    else:
                        data_type_y = None
                    ...
                    if (len(x_values) != len(y_values)):
                        print(file_path, ' ', num_entries_x, ' ',num_entries_y)
                    ...
                    try:
                        ...
                        print("vrethike provlima sta x: ", file_path)
                        print(e)
                        sys.exit(1)
                    #sortedness_y, is_sorted_y = sequence_features(y_values, data_type_y) NA TO FTIAKSWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW

                    try:
                        sortedness_y, is_sorted_y = sequence_features(y_values, data_type_y)
                    except Exception as e:
                        print("vrethike provlima sta y: ", file_path)
                        print(e)
                        sys.exit(1)
                    ...
                    # Store the extracted features and label
                    data = values
                    entry_tuple = tuple(data)
                    if entry_tuple not in unique_entries:
                        unique_entries.add(entry_tuple)  # Add the entry to the set
                        dtv_features.append(values)
                    else:
                        continue
                arithmos = arithmos + 1
                count += 1
        ...
        print("Number Features for", folder_path,":",count)
    
    dtv_columns = ['X_Type', 'Y_Type','Num_Entries_X', 'Num_Entries_Y',
    ...
    df_dtv = pd.DataFrame(dtv_features, columns=dtv_columns)
    
    return df_dtv

# for C = 4 number of outcome classes
folder_paths = ["../data/line","../data/scatter","../data/bar","../data/pie"] #line , scatter , bar, pie
temp_df = process_json_files(folder_paths)
temp_df.to_csv('features.csv', index=False)
dtv = shuffle(temp_df)
dtv.to_csv('c4_lsbp.csv', index=False)
